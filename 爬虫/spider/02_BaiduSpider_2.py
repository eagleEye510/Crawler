...
import sys
import io
from Config_v2 import Config as ConfigV2
    ...
    def run(self, base_url, keywords, offset, href_xpath, max_index, filter_before=True, index=0, sourse_spider=''):
        ...
        while not end:
            ...
            for detail_href in detail_hrefs:
                # try:
                    if filter_before:
                        added = Configs.ToolsObjManager.redis_tool.sadd_value('real_baidu', detail_href)
                        if added:
...
def run_baidu():
    while True:
        ...
        for keyword in keywords:
            ...
            day_url = 'https://www.baidu.com/s?wd=%s&gpc=stf__3D'+str(tm-24*60*60)+'__2C' + str(tm)+'__7Cstftype__3D1&pn=%s'
            ...
            span_list.append(gevent.spawn(spider.run, "http://news.baidu.com/ns?word=%s&pn=%s", keywords=keyword, offset=10,
                       href_xpath='//h3/a/@href', max_index=500, filter_before=False, sourse_spider='百度新闻搜索'))
            # spider = BaiduSpider()
            # span_list.append(
