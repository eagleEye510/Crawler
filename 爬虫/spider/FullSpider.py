...
class FullSpider(object):
    def __init__(self, base_url):
        ...
        if not self.mongo_rules:
            self.mongo_rules = {'site_pattern': ['\.gov\.', ]}
        if not self.mongo_rules.get('forbidden_site_pattern', ''):
            self.mongo_rules['forbidden_site_pattern'] = ['\.exe$', '\.apk$']
        if not self.mongo_rules.get('detail_url_pattern', ''):
            self.mongo_rules['detail_url_pattern'] = ['\d{6}', ]
        if not self.mongo_rules.get('max_deep', 0):
            self.mongo_rules['max_deep'] = 3
        if not self.mongo_rules.get('xpath_list_article', ''):
            self.mongo_rules['xpath_list_article'] = []

        self.mongo_rules['site_pattern'] = Configs.ToolsObjManager.re_tool.re_compiler_list(
            self.mongo_rules['site_pattern'])
        self.mongo_rules['detail_url_pattern'] = Configs.ToolsObjManager.re_tool.re_compiler_list(
            self.mongo_rules['detail_url_pattern'])
        self.mongo_rules['forbidden_site_pattern'] = Configs.ToolsObjManager.re_tool.re_compiler_list(
    ...
    def isvalid_href(self, href):
        ...
        if self.pattern_space.search(href):
            return False
        if Configs.ToolsObjManager.re_tool.href_accord_listpattern(href, self.mongo_rules['forbidden_site_pattern']):
            Configs.MongoConfigs.col_error.insert_one({
                'exit_url': href
            })
            return False
        if Configs.ToolsObjManager.re_tool.href_accord_listpattern(href, self.mongo_rules['site_pattern']):
            return True
        return False
    ...
    def mongo_insert(self, base_url, url, level):
        table_name = self.convert_url2table_name(base_url)
        r = eval('Configs.MongoConfigs.conn.news_url_queue.%s' % table_name)
            ...
            for href in hrefs:
                ...
                if self.isvalid_href(real_href):
                    ...
                    if (not real_href in self.nav_set) and (not repeat):
                        self.mongo_insert(self.base_url, real_href, level)
        except Exception as e:
            return
    ...
    def run(self, url, level):
        ...
        if is_detail:
            ...
            if level >= self.mongo_rules['max_deep']:
                return
            level += 1
